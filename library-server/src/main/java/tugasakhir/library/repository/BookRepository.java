...
public class BookRepository {
    ...
    public List<BookDetail> getBooksByBookTitle(String bookTitle) {
        try{
            bookTitle = "%".concat(bookTitle).concat("%");
            log.info("[GET BOOK DETAIL BY TITLE][{}][{}]", bookTitle, applicationProperties.getGET_BOOKS_DETAIL_BY_TITLE());
            SqlParameterSource paramSource = new MapSqlParameterSource("bookTitle", bookTitle);
            return jdbcTemplate.query(applicationProperties.getGET_BOOKS_DETAIL_BY_TITLE(), paramSource, new BookDetailRowMapper());
        }catch (Exception e){
            log.error(e.getMessage());
